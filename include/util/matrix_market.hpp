...
void loadMatrix(int info, std::istream& fs,
                DSparseMatrixBase<Derived, TVal, TIdx>& target) {
    if (info & matrix_market::info::array) {
        JWLogError << "Trying to load dense matrix .mtx "
                      "file format into a sparse matrix."
                   << endLog;
    }
    ...
    if (info & matrix_market::info::symmetric) {
        for (auto& trip : coefficients) {
            ...
            coefficients.push_back(
                Triplet<TVal, TIdx>(trip.col(), trip.row(), trip.value()));
        }
...
template <typename Derived, typename TVal, typename TIdx>
void loadMatrix(int info, std::istream& fs, DMatrix<TVal, TIdx>& target) {
    if (info & matrix_market::info::coordinate) {
        JWLogError << "Trying to load sparse matrix .mtx "
                      "file format into a dense matrix."
                   << endLog;
    }
...
void loadFromStream(std::istream& fs,
                    DMatrixBase<Derived, TVal, TIdx>& target) {
    std::string header;
    ...
    if (s != "%%MatrixMarket" || t != "matrix") {
        JWLogError << "Not a valid MM file" << endLog;
        exit(1);
    }
    ...
    while (!header_stream.eof()) {
        ...
        if (s == "complex" || s == "Hermitian" || s == "integer") {
