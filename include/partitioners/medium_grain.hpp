...
class MGPartitioner : Zee::IterativePartitioner<TMatrix> {
  public:
    ...
    bool locallyOptimal() const { return locallyOptimal_; }
    ...
    void split(TMatrix& A) {
        ...
        vector<vector<atomic_wrapper<TIdx>>> row_count(
            p, vector<atomic_wrapper<TIdx>>(m / p + 1));
        vector<vector<atomic_wrapper<TIdx>>> col_count(
            p, vector<atomic_wrapper<TIdx>>(n / p + 1));
    ...
    void constructExtendedMatrix(TMatrix& A, TMatrix& B) {
        ...
        for (auto& pimg : images) {
            for (auto t : *pimg) {
                ...
                auto bTriplet =
                    TTriplet(bRow(tripletInAr, t.row(), t.col(), A),
                             bCol(tripletInAr, t.row(), t.col(), A), t.value());
    ...
    inline TIdx bRow(bool tripletInAr, TIdx row, TIdx col,
    ...
    inline TIdx bCol(bool tripletInAr, TIdx row, TIdx col,
                     const TMatrix& A) const {
        if (tripletInAr)
    ...
    void inducePartitioning(TMatrix& A, TMatrix& B) {
        ...
        vector<vector<int>> procForCol(p, vector<int>(B.getRows() / p + 1, -1));
        TIdx s = 0;
        ...
        for (auto& pimg : images) {
            for (auto triplet : *pimg) {
                ...
                if (tripletInRow_[cur++]) {
                    ...
                    targetProc = procForCol[colProc % p][colProc / p];
                } else {
                    targetProc =
    ...
    TMatrix& refine(TMatrix& A) override {
        ...
        for (auto& t : *aImages[0]) {
            auto bTriplet =
                TTriplet(bRow(phaseRow_, t.row(), t.col(), A),
                         bCol(phaseRow_, t.row(), t.col(), A), t.value());
            if (phaseRow_) {
