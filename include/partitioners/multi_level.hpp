...
class MultiLevelOneD : Zee::Partitioner<TMatrix> {
   public:
    ...
    virtual TMatrix& partition(TMatrix& M) override {
        ...
        if (!initialized) {
            JWLogError << "Trying to partition with uninitialized partitioner."
                       << endLog;
            return M;
        }
        ...
        for (TIdx row = 0; row < M.getRows(); ++row) {
            for (auto& pin : rowNets[row]) {
                vertexGains[pin] += gainForPinInRow(
                    columnInA[pin], rowCountA[row], rowNets[row].size());
            }
        }
            ...
            for (auto row : colNets[baseCell]) {
                for (auto pin : rowNets[row]) {
                    verticesToUpdate.insert(
                        std::make_pair(pin, vertexGains[pin]));
                }
            }
        ...
        for (auto& pimg : M.getImages()) {
            for (auto& triplet : *pimg) {
                new_images[(TIdx)(bestSplit[triplet.col()])]->pushTriplet(
                    triplet);
            }
        }
        ...
        JWLogInfo << "FM partitioned into sizes: " << counts[0] << " "
                  << counts[1] << endLog;
