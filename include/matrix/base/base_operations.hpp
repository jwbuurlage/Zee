...
BinaryOperation<operation::type::addition, Derived, OtherDerived> operator+(
    const DMatrixBase<OtherDerived, S, T>& other) const {
    return BinaryOperation<operation::type::addition, Derived, OtherDerived>(
        derived(), other.derived());
}

template <typename OtherDerived, typename S, typename T>
BinaryOperation<operation::type::subtraction, Derived, OtherDerived> operator-(
    const DMatrixBase<OtherDerived, S, T>& other) const {
    return BinaryOperation<operation::type::subtraction, Derived, OtherDerived>(
        derived(), other.derived());
}

template <typename OtherDerived, typename S, typename T>
BinaryOperation<operation::type::product, Derived, OtherDerived> operator*(
    const DMatrixBase<OtherDerived, S, T>& other) const {
    return BinaryOperation<operation::type::product, Derived, OtherDerived>(
        derived(), other.derived());
}
...
BinaryOperation<operation::type::scalar_product, Derived, TVal> operator*(
    const TVal& alpha) const {
    return BinaryOperation<operation::type::scalar_product, Derived, TVal>(
        derived(), alpha);
}

BinaryOperation<operation::type::scalar_division, Derived, TVal> operator/(
    const TVal& alpha) const {
    return BinaryOperation<operation::type::scalar_division, Derived, TVal>(
        derived(), alpha);
}
